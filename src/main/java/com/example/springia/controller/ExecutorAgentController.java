...
public class ExecutorAgentController {
    ...
    public ResponseEntity<String> executeTesteBuilder(
            @RequestParam(defaultValue = "/tmp/tarefas-backend") String path,
            @RequestParam(defaultValue = "clean package") String goals,
            @RequestParam(required = false) String mavenCommand) {
        log.info("[TESTE_BUILDER] GET /teste-builder path={} goals={}", path, goals);
        try {
            Path repoPath = resolveBuilderPath(path);
            String resolvedMavenCommand = resolveMavenCommand(mavenCommand);
            String[] command = buildMavenCommand(resolvedMavenCommand, goals);

            log.info("[TESTE_BUILDER] Executando comando={} diretorio={}", String.join(" ", command), repoPath);
            ProcessBuilderReturnDTO execute = ProcessBuilderUtils.execute(repoPath.toString(), command);
    ...
    private String resolveMavenCommand(String mavenCommand) {
        log.debug("[RESOLVE_MAVEN] Resolvendo comando Maven customizado={}", mavenCommand);

        if (mavenCommand != null && !mavenCommand.isBlank()) {
            return mavenCommand;
        }

        Path sdkmanMaven = Paths.get(System.getProperty("user.home"), ".sdkman", "candidates", "maven", "current", "bin", "mvn");
        if (Files.isExecutable(sdkmanMaven)) {
            return sdkmanMaven.toString();
        }
